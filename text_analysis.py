...
g = open('training.txt', 'r', encoding="utf8")
sentiment = []
text = []
for line in g:
    line = line.split("\t")
...
def getEmojiText(emotion):
    # this can be extended to a list of emocons
    if emotion == "HAPPY":
        return ['😂']
    return ['😅']
			
if __name__ == "__main__":
	mlp = SentimentNetwork(text[:-5000], sentiment[:-5000], learning_rate=0.1)
	mlp.train(text[:-5000],sentiment[:-5000])
	emotion = mlp.run(str(sys.argv))
	print(getEmojiText(emotion))
	#print(mlp.run("this moive is bad")) #uncomment this to see example
	#print(mlp.run("I love you")) #uncomment this to see example